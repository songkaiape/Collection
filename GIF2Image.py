...
def intToBin(i):
    ...
    i2 = int( i/256)
    # 合成小端对齐的字符串
    return str.encode(chr(i1) + chr(i2))
def getheaderAnim(im):
    """ 生成动画文件头 """
    bb = b"GIF89a"
    bb += intToBin(im.size[0])
    bb += intToBin(im.size[1])
    bb += b"\x87\x00\x00"  #使用全局颜色表
    return bb
...
def writeGif(filename, images, duration=0.1, loops=0, dither=1):
    ...
    for im in images:
        ...
        if isinstance(im,Image.Image): #如果是PIL Image
            images2.append(im.convert('P',palette=Image.ADAPTIVE,dither=dither) )
...
if __name__ == '__main__':
    #frames = gif2images('qwe.gif',distDir='tmp',type='png')
    images = []
    for i in range(68,-1,-1):
        images.append('tmp/qwe-%d.png'%i)
